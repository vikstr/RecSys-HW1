...
class Catalog:
    ...
    def load(self, catalog_path, top_tracks_path, new_recommender_tracks_path):
        ...
        self.app.logger.info(f"Loading top tracks from {new_recommender_tracks_path}")
        with open(new_recommender_tracks_path) as new_recommender_tracks_path_file:
            self.user_recommendations = json.load(new_recommender_tracks_path_file)
        self.app.logger.info(f"Loaded recommend tracks for users")
    ...
    def upload_recommendations(self, redis):
        ...
        j = 0
        self.app.logger.info(f"Read recommendations to redis")
        with open(recommendations_file_path) as rf:
            self.app.logger.info(f"Open recommendations to redis")
            for line in rf:
                self.app.logger.info(f"Start to read recommendations to redis")
                recommendations = json.loads(line)
                self.app.logger.info(f"Get recommendations to redis")
                redis.set(
    ...
    def upload_new_user_recommendations(self, new_recommender_tracks):
        self.app.logger.info(f"Uploading recommendations from new recommender")
        j = 0
        for user, tracks in self.user_recommendations.items():
            new_recommender_tracks.set(
                user, self.to_bytes(tracks)
            )
            j += 1
        self.app.logger.info(f"Uploaded recommendations for {j} users")
